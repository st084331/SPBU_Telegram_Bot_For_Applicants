...
with open(f'{config.prefix}intents_ru_mast.json') as file:
    data = json.load(file)


try:
    with open(f'{config.prefix}testdata_ru_mast.pickle', 'rb') as f:
    ...
    with open(f'{config.prefix}testdata_ru_mast.pickle', 'wb') as f:
    ...
    model.save(f'{config.prefix}Model_ru_mast.tflearn')
###################################################
...
try:
   model.load(f'{config.prefix}Model_ru_mast.tflearn')
except:
    model.fit(training, output, n_epoch=10000, show_metric=True)
    model.save(f'{config.prefix}Model_ru_mast.tflearn')
    model.load(f'{config.prefix}Model_ru_mast.tflearn')
