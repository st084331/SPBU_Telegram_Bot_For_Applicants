...
with open('intents_ru_foreingner_asp.json') as file:
    data = json.load(file)


try:
    with open('testdata_ru_foreingner_asp.pickle', 'rb') as f:
    ...
    with open('testdata_ru_foreingner_asp.pickle', 'wb') as f:
...
try:
   model.load('Model_ru_foreingner_asp.tflearn')
except:
    model.fit(training, output, n_epoch=10000, show_metric=True)
    model.save('Model_ru_foreingner_asp.tflearn')
