...
with open(f'{config.prefix}intents_bac.json') as file:
    data = json.load(file)


try:
    with open(f'{config.prefix}testdata_bac.pickle', 'rb') as f:
    ...
    with open(f'{config.prefix}testdata_bac.pickle', 'wb') as f:
    ...
    model.save(f'{config.prefix}Model_bac.tflearn')
###################################################
...
try:
   model.load(f'{config.prefix}Model_bac.tflearn')
except:
    model.fit(training, output, n_epoch=10000, show_metric=True)
    model.save(f'{config.prefix}Model_bac.tflearn')
    model.load(f'{config.prefix}Model_bac.tflearn')
