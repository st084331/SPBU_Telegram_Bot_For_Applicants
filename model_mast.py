...
with open(f'{config.prefix}intents_mast.json') as file:
    data = json.load(file)


try:
    with open(f'{config.prefix}testdata_mast.pickle', 'rb') as f:
    ...
    with open(f'{config.prefix}testdata_mast.pickle', 'wb') as f:
    ...
    model.save(f'{config.prefix}Model_mast.tflearn')
###################################################
...
try:
   model.load(f'{config.prefix}Model_mast.tflearn')
except:
    model.fit(training, output, n_epoch=10000, show_metric=True)
    model.save(f'{config.prefix}Model_mast.tflearn')
    model.load(f'{config.prefix}Model_mast.tflearn')
