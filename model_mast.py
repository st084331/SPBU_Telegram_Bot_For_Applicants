...
with open('intents_mast.json') as file:
    data = json.load(file)


try:
    with open('testdata_mast.pickle', 'rb') as f:
    ...
    with open('testdata_mast.pickle', 'wb') as f:
...
try:
   model.load('Model_mast.tflearn')
except:
    model.fit(training, output, n_epoch=10000, show_metric=True)
    model.save('Model_mast.tflearn')
